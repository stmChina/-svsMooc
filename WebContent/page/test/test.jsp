...
<html>
  ...
  <body>
	...
	<div class="jumbotron">
	  ...
	  <form:form modelAttribute="question" action="${fn:escapeXml(formUrl)}" method="post" class="form-horizontal"
                   id="testform">
	  	<p>
	  		<span class="label label-primary">No.<c:out value="${qNo}"/></span>
	  		<form:hidden path="disOrder"/>
	  	</p>
	  	
	  	
	  	
		<p><c:out value="${question.question.qContent}"/></p>
		
		<c:forEach var="dtl" items="${question.dDtls}">
			<p><input type="radio" id="aid" name="aid" value="${dtl.answer.aid}">${dtl.disorder} : ${dtl.answer.aContent}</p>
       	</c:forEach>
     	<br>
